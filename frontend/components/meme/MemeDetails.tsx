...
import { BarChart3, LineChart, Flame, ThumbsUp, TrendingUp, Trophy, Info, ArrowUpRight, ArrowDownRight, Minus, Calendar, DollarSign, Activity, Volume2 } from "lucide-react";
...
const recentTrades = [
  { id: 1, type: 'buy', amount: 25000, price: 0.052, time: '10 mins ago', wallet: '0x8f23...4a21' },
  { id: 2, type: 'sell', amount: 12000, price: 0.051, time: '25 mins ago', wallet: '0x3e7b...9c45' },
  { id: 3, type: 'buy', amount: 50000, price: 0.050, time: '42 mins ago', wallet: '0xf12c...6d78' },
  { id: 4, type: 'buy', amount: 8000, price: 0.049, time: '1 hour ago', wallet: '0x2a9d...1e34' },
  { id: 5, type: 'sell', amount: 30000, price: 0.048, time: '2 hours ago', wallet: '0x7b3c...8f12' },
];
              ...
              <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle className="flex items-center gap-2">
                    <span className="font-bold text-xl">${profile.tokenSymbol} Price Chart</span>
                    <Badge 
                      className={`${priceChange.isPositive ? 'bg-green-500 hover:bg-green-500' : 'bg-red-500 hover:bg-red-500'} text-white`}
                    >
                      {priceChange.isPositive ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />}
                  ...
                  <Tabs defaultValue="chart" className="w-full">
                    ...
                    <TabsContent value="chart" className="space-y-4">
                      <div className="flex justify-between items-center">
                        <div>
                          <h3 className="text-2xl font-bold">{profile.tokenPrice} ETH</h3>
                          <div className="flex items-center gap-1 text-sm">
                            <span className={priceChange.isPositive ? 'text-green-500' : 'text-red-500'}>
                              {priceChange.isPositive ? '+' : '-'}{priceChange.value.toFixed(5)} ETH
                              ({priceChange.percentage.toFixed(2)}%)
                            </span>
                            <span className="text-gray-500">Past {chartTimeframe}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                          {['24h', '7d', '30d', '90d'].map(timeframe => (
                            <Button 
                              ...
                              size="sm"
                              className={chartTimeframe === timeframe ? 'bg-primary hover:bg-primary/90' : ''}
                          ...
                          <div className="w-full h-full flex flex-col">
                            <div className="flex-1 relative">
                              ...
                              <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
                                <path 
                                  d={`M0,${100 - parseFloat(filteredData[0]?.price) * 1000} ${filteredData.map((point, i) => 
                                    `L${(i / (filteredData.length - 1)) * 100},${100 - parseFloat(point.price) * 1000}`
                                  ).join(' ')}`}
                                  fill="none"
                              ...
                              <div className="absolute top-0 right-0 p-2 text-sm text-gray-500">
                                {Math.max(...filteredData.map(d => parseFloat(d.price))).toFixed(5)} ETH
                              </div>
                              <div className="absolute bottom-0 right-0 p-2 text-sm text-gray-500">
                                {Math.min(...filteredData.map(d => parseFloat(d.price))).toFixed(5)} ETH
                              </div>
                            </div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <Card>
                          <CardContent className="p-4">
                            ...
                            <p className="font-bold">
                              {(profile.circulatingSupply * parseFloat(profile.tokenPrice)).toLocaleString()} ETH
                            </p>
                          </CardContent>
                        </Card>
                        ...
                        <Card>
                          <CardContent className="p-4">
                            ...
                            <p className="font-bold">
                              {(Math.floor(Math.random() * 50000) + 10000).toLocaleString()} ETH
                            </p>
                          </CardContent>
                        </Card>
                    ...
                    <TabsContent value="trades">
                      <div className="border rounded-md overflow-hidden">
                        ...
                        {recentTrades.map(trade => (
                          <div key={trade.id} className="grid grid-cols-5 gap-4 p-3 border-b last:border-b-0 text-sm">
                            <div>
                              <Badge className={trade.type === 'buy' ? 'bg-green-500 hover:bg-green-500' : 'bg-red-500 hover:bg-red-500'}>
                                {trade.type.toUpperCase()}
                              </Badge>
                            </div>
                            <div>{trade.amount.toLocaleString()} ${profile.tokenSymbol}</div>
                      ...
                      <div className="mt-4 p-4 bg-gray-50 rounded-lg border text-sm text-gray-500 flex items-start gap-2">
                        ...
                        <p>
                          All trades are executed on-chain. The price is determined by the current liquidity pool ratio.
