...
export function useHasStaked({
  ...
  const { data, isLoading, error, refetch } = useReadContract({
    ...
    args: [
      ...
      tokenAddress ? (tokenAddress.startsWith('0x') ? tokenAddress : `0x${tokenAddress}`) as Address : '0x0000000000000000000000000000000000000000' as Address
    ],
    query: {
      enabled: enabled && !!userAddress && !!tokenAddress && isAddress(userAddress as string) && isAddress(tokenAddress as string),
    },
  });
